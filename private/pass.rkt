...
(begin-for-syntax
  (define-syntax ~define-pass
    (pattern-expander
     (lambda (stx)
       (syntax-parse stx
         [(_ name:id ilang:id olang:id e:id fml:id erv:id rest)
          #'(~seq (~var name id) (~datum :) (~var ilang id) (~datum ->) (~var olang id)
                  (~or (~optional (~seq #:input (~var e id)) #:defaults ([e #'e]))
                       (~optional (~seq #:formals ((~var fml id) (... ...)))
                                  #:defaults ([(fml 1) null]))
                       (~optional (~seq #:extra-return-values ((~var erv id) (... ...)))
                                  #:defaults ([(erv 1) null])))
                  (... ...)
                  rest (... ...))])))))
...
(define-syntax (trace-define-pass x)
  (define unparser
    (lambda (lang)
      (cond
  ...
  (syntax-parse x
    [(_ (~define-pass name ilang olang in id* xtra body))
     #:with (id ...) #`(in id* ...)
     (let ([iunparser (unparser #'ilang)] [ounparser (unparser #'olang)])
       #`(define (name id ...)
           (define-pass name : ilang -> olang
             #:input in
             #:formals (id* ...)
             #:extra-return-values (xtra ...)
             body ...)
           (let ([tpass name])
             #,(if iunparser
                   (if ounparser
                       (with-syntax ([(ot xvals ...) (generate-temporaries #'(name xtra ...))]
                         ...
                         #`(let ([result #f])
                             (trace-let name ([tid (#,iunparser in #t)] [xargs id*] ...)
                                        (let-values ([(ot xvals ...) (tpass in id* ...)])
...
(define-syntax (echo-define-pass x)
  (syntax-parse x
    [(_ (~define-pass name ilang olang in fml xval more))
     #'(x-define-pass name : ilang -> olang
                      #:in in
                      #:formals (fml ...)
                      #:extra-return-val (xval ...)
                      (expand-modifier echo) more ...)]))

(define-syntax (x-define-pass x)
  ...
  (define (parse-proc pass-name ilang olang)
    (lambda (x)
      (let loop ([x x] [trace? #f] [echo? #f])
        (syntax-parse x
          [((~datum echo) (~and nc (~not (~datum :))) . rest)
           (loop #'(nc . rest) trace? #t)]                  
          [((~datum 'trace) (~and nc (~not (~datum :))) . rest)
           (loop #'(nc . rest) #t echo?)]
          [((~define-pass proc-name itype otype in arg rv body))
  ...
  (define (build-checked-body pass-desc maybe-fml xval* maybe-itype maybe-otype maybe-ometa-parser maybe-body)
    (define (generate-output-check type x ntspec*)
    ...
    (define (generate-body maybe-olang maybe-otype)
      (cond
  ...
  (define (do-define-pass pass-name echo? maybe-iname maybe-itype fml* maybe-oname maybe-otype xval* defn* p* maybe-body)
    (define echo-pass
      (lambda (x)
        (when echo?
  ...
  (syntax-parse x
    [(_ (~define-pass pass-name iname oname in fml xval stuff))
     (let ([squawk (lambda (msg what) (raise-syntax-error who msg x what))])
       (let-values ([(maybe-iname maybe-itype)
                     (syntax-parse #'iname
                       [(~datum *) (values #f #f)]
                       [iname:id (values #'iname #f)]
                       [(iname:id itype:id)
                        (values #'iname #'itype)]
                       [_ (squawk "invalid input language specifier ~a" #'iname)])])
         (let ([fml* (stx->list #'(in fml ...))])
               ...
               (if (stx-null? stuff*)
                   ...
                   (let ([stuff (stx-car stuff*)])
                     (if (let processor? ([stuff stuff] [mcount 0])                        
                           (syntax-parse stuff
                             [((~define-pass pname itype otype in fml xval more)) #t]
                             [(?modifier (~and nc (~not (~datum :))) . more)
                              #:when (and (memq (datum ?modifier) '(trace echo))
                                          (< mcount 2))
                              (processor? #'(nc . more) (+ mcount 1))]
                             [_ #f]))
