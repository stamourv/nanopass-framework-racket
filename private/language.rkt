...
(define-syntax (define-language x)
  ...
  (define (partition-ntspecs ntspecs terminal-meta*)
  ...
  (define parse-alts
    (lambda (alt* terminal-meta*)
      ...
      (let f ([alt* alt*])
        (syntax-parse alt*
          [() '()]
          [(((~literal/datum =>) syn pretty) . alt*)
           (cons (make-alt #'syn #'pretty #f) (f #'alt*))]
          [(syn (~literal/datum =>) pretty . alt*)
           (cons (make-alt #'syn #'pretty #f) (f #'alt*))]
          [(((~literal/datum ->) syn prettyf) . alt*)
